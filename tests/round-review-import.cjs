...
const old={...feed,entries:feed.entries.filter(e=>!['bag-restore-full-set-by-id-20260924','round-tm-ballybunion-20260915-review-v1','lesson-sim-approach-repeatability-20260915','round-review-club-map-20260915-v1','round-review-complete-shots-20260915-v2','bay-delivery-20260914-v1','bay-map-repair-20260915-v2'].includes(e.id))};
...
const r=T.get().rounds.find(r=>r.course==='Ballybunion');
assert.equal(T.get().rounds.filter(x=>x.review).length,2);
assert.equal(T.get().rounds.filter(x=>x.sim).length,3);
assert.equal(T.get().rounds.filter(x=>x.course==='Spyglass Hill').length,1);
assert.equal(T.get().rounds.filter(x=>x.course==='Hazeltine National').length,1);
...
assert.ok(T.bag().includes('The record'));
assert.ok(T.bag().includes('Cumulative'));
const threeWood=T.get().clubs.find(c=>c.name==='Cobra DS-ADAPT X 3-wood');
assert.equal(threeWood.futureFit.current,'A3');
assert.deepEqual(JSON.parse(JSON.stringify(threeWood.futureFit.target)),{code:'B7',loft:1.7,lie:-0.7,purpose:'Higher-launch test while remaining slightly flat'});
assert.ok(threeWood.note.includes('IN Sep 18 2026'),'FutureFit patch preserves the existing club note');
const bagHtml=T.bag();
const set=T.get().clubs.find(c=>c.id==='c6');
assert.equal(set.name,'Cobra KING TEC Irons · 4–PW');
assert.ok(set.spec.startsWith('4–PW'));
assert.ok(!set.note.includes('THE 4-IRON CAME OUT'));
assert.equal(T.get().clubs.filter(c=>c.id==='bag-kingtec-4iron-20260924').length,0);
assert.ok(T.get().clubs.some(c=>c.name==='Cobra KING TEC Utility 2-iron' && c.status==='gaming'));
assert.ok(bagHtml.includes('14 CLUBS'));

assert.ok(bagHtml.includes('14° effective'));
assert.ok(bagHtml.includes('16.7° effective'));
assert.ok(bagHtml.includes('futurefit33-rh.jpg'));
const mapBay = T.get().bays.findIndex(b => b._fid === 'bay-20260914-gl18-map-my-bag');
const bayHtml = T.bayView(mapBay);
assert.ok(!bayHtml.includes('Simulator Round Review'));
assert.ok(bayHtml.includes('Carry vs total') || bayHtml.includes('Carry shape'));
...
assert.ok(T.bayView(T.get().bays.indexOf(repairedBay)).includes('Carry vs total') || T.bayView(T.get().bays.indexOf(repairedBay)).includes('Carry shape'));
...
assert.ok(T.sessionLibrary('range').indexOf('<b>Range practice · 54 shots</b>')<T.sessionLibrary('range').indexOf('<b>Map My Bag</b>'));
...
assert.ok(T.sessionLibrary('sim').includes('Spyglass Hill'));
assert.ok(!T.sessionLibrary('outdoor').includes('Spyglass Hill'));
assert.ok(!T.sessionLibrary('range').includes('Spyglass Hill'));
assert.ok(!T.sessionLibrary('outdoor').includes('Ballybunion'));
for(const k of ['days','cumulative']) assert.ok(T.sessionShortcuts().includes('data-kind="'+k+'"'));
...
assert.ok(sorted.includes('Newer') && sorted.includes('Older'));
T.get().bays=bayOrder;
