...
function livePlay(L){
  ...
  <div class="card lvcard">
    ...
    ${row('Putts', `<div class="chips">${[0,1,2,3,4,5].map(p =>
      chip('putts', p, p === 5 ? '5+' : p, h.putts === p)).join('')}</div>`, 'the total')}
    ${!h.putts ? '' : row('Putt made', `<div class="chips">${PUTT_DIST.map(d =>
      chip('pm', d.k, d.lab, h.pm === d.k && !h.gimme)).join('')}
      ${chip('gimme', '1', 'Given', !!h.gimme, 'ns')}</div>`,
      'feet — how long the one you holed was')}
    ${h.putts >= 2 ? row('First putt', `<div class="chips">${PUTT_DIST.map(d =>
      chip('pd', d.k, d.lab, h.pd === d.k)).join('')}</div>`,
      'feet — where you started from') : ''}
