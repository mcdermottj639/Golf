...
On-course rounds keep every shot. Simulator club comparisons now apply the separate v157 exclusion below; their scorecards and raw shot ledgers still retain every shot. A displayed TrackMan average that still has the
...
2. **A fat 9-iron at 70% of the mean is not a mishit.** Two shots are not a median.
   `isClearMishit()` (v130) takes the median of the cluster that already got up
   (carry ≥ half the raw median) and cuts anything under **two-thirds of that
   cluster**. That drops a 5-wood at 112 yards / 8′ of apex without eating the
   fat 9-iron. Other tops must not drag the median down. Without per-shot rows,
   a displayed average cannot be cleaned — say so, do not guess.
...
4. **Path and face rings use the same cut.** `analysisDelivery()` rebuilds the rings
   ...
   off-screen until the carry column is in (Sep 18 7-iron now has carries: 131.7 n=11). Cumulative's path-across-days uses
   that, not the polluted club average.
...
| 3-wood | **Cobra DS-ADAPT X · 15° stock** | **IN Sep 18 2026.** Golf Galaxy same-day, SKU 26808014, $199. Right hand, Regular, **Project X Denali Blue 60** graphite (Cobra's chart: 64g, mid launch). Replaced the mini in the 14. Jack, Sep 18 night: the **2-iron is the fairway finder**; **3-wood or 5-wood if it needs to be high or draw**. Sep 19 morning: 2-iron ~215; **past 360, 3-wood or 5-wood** — working rule, still figuring it out. **Stock 15.0°, not the 16.5° 3-HL.** FutureFit33 unread. Playing length unmeasured (stock 43.25"). **Live carry still blank.** First look the same day at Golf Lounge 18: analysis **176.5 n=9 struck**. TrackMan's screen 150.0 n=11 had two tops in it (26.4, 34.9) — those shots stay on the day, held out of the average. **150 is not on the row.** Bay offer is 176.5, not 150. Jack: *felt amazing*. **After slot (trail hand, later the same day):** struck **188.4 n=7** (screen 150.3 n=9 had tops 21.5 and 12.6 in it). Path **−6.1° → −2.0°**. Do not park the Sep 14 TrackMan `3w` 182.4 (that was the mini) |
...
altitude were not shown, so each figure is parked as a visible bay offer. The TrackMan `3w` label was mapped to the Mini Driver on Sep 15 for the Ballybunion sim round. The Mini left the 14 on Sep 18; **do not park 182.4 on the new DS-ADAPT X 3-wood**. Before Sep 14,
...
`S.carries` is the real 13-club list: `driver`, `3-wood`, `2-iron`, `5-wood`, `5-iron`…
...
current bag (2-iron is the fairway finder at ~215, his number; working rule: inside 360, 2-iron; past 360, 3-wood or 5-wood; wood if it needs to be high or draw either way. Still figuring it out). Hazard notes on 2 / 10 / 12 cite a 1979
...
### Six-metric TrackMan baseline (v113 · Sep 17 2026)

The dedicated improvement fields are **Ball Speed, Smash Factor, Face Angle, Face-to-Path, Spin Rate, and Carry Distance**. The Caddie HQ card must define each number and interpret the actual baseline in plain language: ball speed is output, smash is strike efficiency, face angle is start direction, face-to-path is curve control, spin governs flight/roll/curve, and carry is air distance. The ball-flight read must follow the signs: −1.6° face with +6.5° face-to-path describes a left-starting ball that peels right (pull fade / bigger cut), **not** a draw or hook. The original captured panel is one unconfirmed-club TrackMan shot: 128.5 mph ball speed, 1.42 smash, −1.6° face angle, +6.5° face-to-path, 3,979 rpm spin and 193.7 yd flat carry. It is delivered through `front9-feed.json` alongside the Sep 16 simulator card because the existing main feed is too large to safely replace through the connected publisher. `fetchFeed()` loads both feeds, and the service worker caches both. This is a single-shot field baseline, not a personal average, a club baseline, a fitting conclusion or a carry-ladder update.
...
The original Sep15 play-date label was corrected to Sep22 in v155 after Jack confirmed
the day played; the old feed ID remains stable for existing installs. Keep simulator results out of outdoor stats
...
**v155 Spyglass play date.** Jack confirmed the Spyglass 85 was played September 22, 2026, and was his first qualifying TrackMan HCP round. A TrackMan Golf profile screenshot shows HCP 7.4 after one round; store it as a dated TrackMan snapshot and never as the outdoor handicap. The Sep 15 date in the original feed and v154 source repair was a legacy guess from an earlier card. The canonical `data/spyglass-verified-2026-09-22.json` uses the confirmed day. A new `round-update` entry in `corrections-20260922.json` targets the stable original `feedId`, so phones which already applied the earlier correction receive the play-date fix. Simulator day chips show SIM; the Spyglass row additionally says TrackMan HCP. Indoor rounds remain excluded from outdoor scoring while the Spyglass round retains its TrackMan HCP status. Do not rename the historical feed ID to encode the corrected date.

**v156 Spyglass bay profiles.** The old round-wide noBay guard blanked all six comparison rows because of one Mini/3-wood concern. Spyglass now has explicit bayClubLinks keyed by round club, each naming a stable bay feed ID and exact source block. The 3-wood and 5-wood use Sep18 later blocks, 5i/7i Sep22, 9i/56 Sep15. roundReviewBays() passes analysisClubs() results to the review so remaining-shot counts and means use the same rules as Days and Cumulative. Historical Mini labels stay historical. A missing explicit source must stay pending, never fall back to a different head, block, or later session. Every comparison shows its source date/block; same-day practice/round order is unknown. The fresh review-update ID reaches already imported phones. Jack confirmed current 3-wood; Mini is out of his active bag (already recorded since v119).


**v157 comparison exclusions.** Jack asked to remove obvious mishits (the short 3-wood example) from range-versus-round profiles. `round-review.js` now derives comparison-only exclusions per confirmed club from unflagged, explicitly full simulator shots. Explicit `mishit:true` is held out; otherwise use measured numeric carry below two-thirds of the cluster median, where the cluster is carries at least half the raw median and must contain at least three readings. Missing carry, planning yardage, partial/recovery/unknown shots, other clubs, other rounds, and bay references cannot drive this filter. The very short result is an outlier, not proof of contact cause (a collision or other course condition can shorten it). Ordinary misses remain. Distance and path use the same retained cohort with their own missing-metric handling; the table shows used/eligible counts and exclusions, and the ledger marks excluded rows. Spyglass excludes H7 43.8 and H10 89.0 carry, retaining H13 137.8; 3-wood becomes 186.0 yd flat carry / -1.2° path, 14 of 16 full shots. The H7 shot-list total is 75; never overwrite its verified 43.8 carry with that planning field. All 57 observations, score 85, HCP status, findings and outdoor statistics remain unchanged. This is derived rendering logic so existing installs receive it without rewriting historical feeds. Range filtering remains in `analysisClubs()`; do not delete raw round shots or use the cleaned comparison to change playing carries. Regression: `tests/round-comparison-mishits.cjs`.


**v158 Spyglass shot-list distances.** Jack confirmed his Hole 1 TrackMan screenshot: a 3w row labelled "248yds to rough" means 248 yd TOTAL distance after roll, the separate flat carry tile is 189.4 yd in the air, and "300yds to hole" is what remains. The original importer mistakenly set `distance` from `carry` and filed the 248 shot-list distance under misleading legacy name `planYds`. The shot-list labels for all 57 Spyglass observations were already transcribed as `planYds`; the canonical verified source now sets `distance` from that row label, preserving `planYds` as source history, flat carry, remaining distance, club, metric panels, score and every row. The comparison columns explicitly separate **Round carry** and **Round total**, so no one compares them as one measure; the 3w retained-shot mean is 210.1 yd in that column, versus the 186.0 yd mean of the separate flat carry tiles. A fresh append-only `round-shot-distance-update` entry supplies an audited 57-ID mapping to existing installs. The importer patches only `distance` by matching shot ID and preserves local identity overrides and all other fields. The correction does not infer distances from hole length minus remaining yards or from a carry-to-total formula. Other shot-list totals can be shorter than flat carry on slopes, hazards or spin; do not infer roll amount from one screen or compute total from carry. Hazeltine already preserves separate shot-list totals and flat carries, while Ballybunion has shot-list distances without verified carry. The Sep16 virtual front nine had only seven flat-carry tiles: a separate targeted correction clears its seven duplicated `distance` fields, preserving the carries; mark that review `carry-only`. Its shot heading displays the destination and carry tile without inventing a total. Both source corrections are append-only and apply to existing phones. Verify `tests/spyglass-shot-distance.cjs` and the 320/390 browser round review.
