...
| 3-wood | **Cobra DS-ADAPT X · 15° stock** | **IN Sep 18 2026.** Golf Galaxy same-day, SKU 26808014, $199. Right hand, Regular, **Project X Denali Blue 60** graphite (Cobra's chart: 64g, mid launch). Takes the fairway-finder slot the mini driver held. **Stock 15.0°, not the 16.5° 3-HL.** FutureFit33 can move loft and lie independently ±2°, and nobody has read the sleeve, so treat 15 as the box spec rather than the hosel — same open question as the driver's adapter. Playing length unmeasured (stock 43.25"). **Carry blank on purpose.** Do not park the Sep 14 TrackMan `3w` 182.4 on this head — that label was the mini. Loft arithmetic against the 2-iron (~17°) is two degrees, outside the 1.5° OVERLAP flag |
...
| Irons | Cobra KING TEC **5–PW** | **the 4-iron came out Sep 8 2026** to hold the bag at 14 when the mini went back in, and **stays out Sep 18** now the 3-wood is in — kept with the set, not sold. **44° PW** anchors the wedge ladder |
