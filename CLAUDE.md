...
4. **Path and face rings use the same cut.** `analysisDelivery()` rebuilds the rings
   ...
   off-screen (Sep 18 7-iron: 11 paths, 0 carries). Cumulative's path-across-days uses
...
| Irons | Cobra KING TEC **5–PW** | **the 4-iron came out Sep 8 2026** to hold the bag at 14 when the mini went back in, and **stays out Sep 18** now the 3-wood is in — kept with the set, not sold. **44° PW** anchors the wedge ladder. **Sep 18 bay:** 7-iron displayed 131.7 n=11 cons 12.4 (Sep 14 was 135.9 / 12.5). Per-shot carries were off-screen so that figure cannot be cleaned. Delivery is complete: path −6.9°, face-to-path **+6.4°** (most open of 3w/5w/7i that day). v123 puts those 11 shots on the day and in the Cumulative rings |
