...
(function(root){
  ...
  function render(r,ctx){
    ...
    return `<section class="rr" aria-label="Simulator Round Review">
      ...
      <div class="tscroll"><table><thead><tr><th>Club</th><th>Bay carry yd</th><th>Bay path °</th><th>Round distance yd</th><th>Round path °</th><th>Full shots used</th></tr></thead><tbody>${ps.map(p=>`<tr><td>${esc(label(p.club))}${p.bayLabel ? `<div class="sm faint">${esc(p.bayLabel)}${p.bay ? ` · n=${p.bay.n}` : " · source pending"}</div>` : ""}</td><td>${num(p.bay?.carry)}</td><td>${num(p.bay?.path)}</td><td>${num(p.distance)}</td><td>${num(p.path)} <small>(n=${p.pathN})</small></td><td>${p.n} / ${p.fullN}${p.excluded.length?`<div class="sm faint">${p.excluded.length} excluded</div>`:''}</td></tr>`).join('')}</tbody></table></div>
