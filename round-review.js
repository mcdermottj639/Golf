...
(function(root){
  ...
  const label = k => ({driver:'Driver','3-wood':'3w · TrackMan label','5-wood':'5-wood','2-iron':'2-iron','6-iron':'6-iron','9-iron':'9-iron','56-wedge':'56°','60-wedge':'60°'}[k] || k);
  ...
  function findings(r){
    ...
    return [
      ...
      {title:'Delivery is not one fixed pattern',body:`Among ${paths.length} unflagged captured path readings: ${neg} negative, ${pos} positive, ${paths.length-neg-pos} neutral. This mixes short and long shots and is not a full-swing diagnosis. Actual clubs and swing intent remain unconfirmed. Judge delivery alongside the result; these readings cannot establish a body-mechanics cause.`,holes:[...new Set(paths.map(s=>s.hole))]}
    ];
  }
  function hub(rounds){
  ...
  function render(r,ctx){
    ...
    return `<section class="rr" aria-label="Simulator Round Review">
      ...
      <p class="sm faint">Bay: latest matching club on or before this round. Round profiles require confirmed actual club and full-swing intent. Zero means no eligible identified shots—not no shots hit. TrackMan 3w is not automatically mapped to Mini Driver because labels were sometimes stale.</p></div>
      ...
      <details class="card"><summary>Sources & unresolved differences</summary><p class="sm">TrackMan: average drive 217 yd, longest 245 yd, scrambling 13%, assigned putts 29. Companion-app recording: driving distance 224.34 (unit not visible in captured panel), scrambling 66.67% (10/15), total strokes gained −29.76 with benchmark unconfirmed. These are source reports, not reconciled statistics; no combined strokes-gained diagnosis is made.</p><p class="sm">${esc(q.coverage)}</p></details>
    </section>`;
  }
